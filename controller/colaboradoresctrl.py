...
class ColaboradorCtrl:
    ...
    def salvar_atualizar_departamento(self, nome,endereco,sexo,data_nasci,salario,fk_departamentos_id, id=None):
        try:
            d = Departamentos.get(nome=fk_departamentos_id)
            ...
            if id:
                ...
                colaborador.fk_departamentos_id = d
            else:
                colaborador = Colaboradores(nome=nome,endereco = endereco,sexo = sexo,data_nasci = dn,salario = s,fk_departamentos_id = d)
            colaborador.save()
    ...
    def buscar_colaborador(self, id=None, nome=None):
        try:
            if id:
                colaborador = Colaboradores.get_by_id(id)
            elif nome:
        ...
        if type(colaborador) is Colaboradores:
            itens.append(
                self._montar_colaborador(colaborador.id, colaborador.nome, colaborador.endereco, colaborador.sexo,
                                         ...
                                         colaborador.fk_departamentos_id.nome))
    ...
    def _montar_colaborador (self, id, nome,endereco,sexo,data_nasci,salario,fk_departamentos_id):
        ...
        todoscolaboradores = {
            ...
            'lblDepartamento': self._criarLabel(fk_departamentos_id, 0.4),
    ...
    def buscarDepartamentos(self):
        ...
        for departamento in departamentosbanco:
            departamentos.append({"id": departamento.id, "nome": departamento.nome})
        return